...
/// <summary>
/// 重新设置table的大小
/// 说明：luaH_new函数仅仅是初始化了一个table，真正table容器大小，需要调用次方法实现
/// </summary>
/// <param name="L"></param>
/// <param name="t"></param>
/// <param name="nasize">数组节点的大小</param>
/// <param name="nhsize">hash节点的大小</param>
...
TValue* luaH_newkey(lua_State* L, Table* t, const TValue* key) {
	Node* mp;
	TValue aux;
	if (ttisnil(key)) luaG_runerror(L, "table index is nil"); // 检查key是否为空置
	...
	if (!ttisnil(gval(mp)) || isdummy(t)) {  /* main position is taken? */
		...
		othern = mainposition(t, gkey(mp)); ////拿到key 可以存放的的node
...
*   说明：
*   优先在t->array数组上查询，是否有节点可以存储，如果key小于arraysize，则放在array上
