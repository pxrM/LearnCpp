...
LUALIB_API void luaL_requiref(lua_State* L, const char* modname,
	...
	//在全局注册表G(L)->l_registry上创建一个LUA_LOADED_TABLE的表。如果该表已经创建，则不重复创建。并将该表放入L->top栈顶。
	luaL_getsubtable(L, LUA_REGISTRYINDEX, LUA_LOADED_TABLE);
	//通过lua_getfield方法，拿着modname模块名称去取，是否存在该key值。如果不存在，则返回nil。
	lua_getfield(L, -1, modname);  /* LOADED[modname] */
	if (!lua_toboolean(L, -1)) {  /* package not already loaded?  检查模块是否已经重新加载过。如果没有加载过（nil值返回false），则重新加载模块。 */
